...
# choice of dev|prod
ENVIRONMENT=dev
DEPLOYMENT_NAME=deployAllResources
...
REGION=$(cat $PARAM_FILE | jq -r .parameters.region.value)
RG_NAME="rg-${APP_NAME}-${REGION}-${ENVIRONMENT}-002"
...
az deployment group show --name $DEPLOYMENT_NAME -g $RG_NAME --query properties.outputs
